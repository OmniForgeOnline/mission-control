...
describe("workflows api", () => {
  ...
  it("returns a full definition by id and 404 for unknown", async () => {
    const app = createServer({ root });
    const ok = await request(app).get("/api/workflows/code-feature");
    expect(ok.status).toBe(200);
    expect(ok.body.id).toBe("code-feature");
    expect(ok.body.defaults.author).toBe("claude");

    const missing = await request(app).get("/api/workflows/does-not-exist");
    expect(missing.status).toBe(404);
  });

  it("creates a new workflow, persists it to YAML, and rejects duplicate ids", async () => {
    const app = createServer({ root });
    const created = await request(app).post("/api/workflows").send(buildWorkflow("my-flow"));
    ...
    const fetched = await request(app).get("/api/workflows/my-flow");
    expect(fetched.status).toBe(200);

    const duplicate = await request(app).post("/api/workflows").send(buildWorkflow("code-feature"));
  ...
  it("rejects an invalid workflow without writing a file", async () => {
    ...
    const res = await request(app).post("/api/workflows").send(invalid);
  ...
  it("updates an existing workflow and invalidates the cache", async () => {
    const app = createServer({ root });
    await request(app).post("/api/workflows").send(buildWorkflow("editable"));

    const updated = buildWorkflow("editable");
    updated.name = "Edited Name";
    const res = await request(app).put("/api/workflows/editable").send(updated);
    expect(res.status).toBe(200);
    expect(res.body.name).toBe("Edited Name");

    // Cache must have been reset so the next read reflects the new name.
    const reread = await request(app).get("/api/workflows/editable");
  ...
  it("deletes a workflow but protects the default workflow", async () => {
    const app = createServer({ root });
    await request(app).post("/api/workflows").send(buildWorkflow("deletable"));

    const removed = await request(app).delete("/api/workflows/deletable");
    expect(removed.status).toBe(204);
    const after = await request(app).get("/api/workflows/deletable");
    expect(after.status).toBe(404);

    const protectDefault = await request(app).delete("/api/workflows/code-feature");
    expect(protectDefault.status).toBe(400);
  });

  it("dry-runs validation without writing", async () => {
    const app = createServer({ root });
    const valid = await request(app).post("/api/workflows/validate").send(buildWorkflow("preview"));
    ...
    const invalid = await request(app).post("/api/workflows/validate").send(bad);
  ...
  it("round-trips a definition through serialize and parse", async () => {
    ...
    const ser = await request(app).post("/api/workflows/serialize").send(def);
    ...
    const broken = await request(app).post("/api/workflows/parse").send({ yaml: "id: x\nsteps: {}\n" });
