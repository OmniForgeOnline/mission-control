...
- **MC writes its own managed state plus a per-launch run-dir config — with one exception.** Grep confirms MC writes `data/state/settings.json`, `data/state/agent-config.json`, `data/state/stage-agents.json`, and a per-launch run-dir MCP config for claude/codex/opencode. It never writes `~/.claude/settings.json`, `~/.codex/config.toml`, etc. **The grok adapter is the existing exception:** `writeMcpConfig`'s grok branch calls `ensureGrokMcp`, which runs an imperative `grok mcp add gbrain ...` (`src/mcp/launcher.ts:113-122`) that persistently mutates grok's own MCP registry. It is invoked both at server startup (`src/server/index.ts:41`) and per launch (`src/mcp/launcher.ts:141`), so it is not worktree-scoped, not under `data/state/`, and not a run-dir config file. The new scoping design extends the worktree-scoped pattern (claude/codex) and **must avoid replicating grok's global-mutation approach** for plugin/skill enablement; if grok is ever brought into the scoping fold, its adapter should first be migrated off imperative `mcp add` (see Phase-0 risk note).
...
- **Phase 0 (de-risk, no UI):** empirical spikes. (a) Confirm a worktree-local `.claude/settings.local.json` with `"claude-seo@...": false` actually removes the seo subagents/skill from a Claude Code session's context. (b) Confirm codex `-c` can toggle `plugins.*.enabled` and `skills.config` per launch and that disabled skills drop from context. (c) Confirm the run-dir/worktree boundary each adapter sees as CWD. These three checks gate everything; if any fails, the mechanism changes. (d) Note the existing anti-pattern: grok's adapter mutates grok's global MCP registry via imperative `mcp add` (`src/mcp/launcher.ts:113-122`, invoked at `src/server/index.ts:41` and per launch); if grok is ever brought into the scoping fold, migrate it off `mcp add` to a per-launch/worktree-scoped config first.
...
Risks:
- **Global mutation footgun.** Any path that edits `~/.claude/settings.json` or `~/.codex/config.toml` directly must be rejected by design. The worktree-scoped per-launch config is the mitigation; the registry's "disable" must mean "off for this launch/scope," not "rewrite global config." The one exception is a deliberate, operator-confirmed global install/uninstall. Note that grok's adapter already demonstrates this anti-pattern today (imperative `mcp add` mutating grok's global MCP registry); the new design must not replicate it, and grok should be migrated off `mcp add` before being scoped.
